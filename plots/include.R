...
read_from_archive <- function(.dir, .archive, .rm = FALSE) {
    archive_path <- paste0(.dir, "/", .archive, ".tar.gz")
    folder_path <- paste0(.dir, "/", .archive)

    untar(archive_path, exdir = .dir, extras = "--force-local")

    all_csv_files <- read_all_csv(folder_path)
